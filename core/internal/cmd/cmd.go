...
var (
	Main = gcmd.Command{
		...
		Func: func(ctx context.Context, parser *gcmd.Parser) (err error) {
			...
			s.BindHookHandlerByMap("/*", map[ghttp.HookName]ghttp.HandlerFunc{
				ghttp.HookBeforeServe: func(r *ghttp.Request) {
					...
					if safepath != "" {
						...
						if strings.HasPrefix(r.URL.Path, "/roundcube") || strings.HasPrefix(r.URL.Path, "/.well-known/acme-challenge") {
							return
						}
			...
			// Add PHP-FPM middleware (safely buffer body for all mutating methods to prevent body draining)
			...
			// Handle trailing slash redirect for /roundcube
